...
import {
  Document as DocxDocument, Packer as DocxPacker,
  Paragraph as DocxParagraph, TextRun as DocxTextRun,
  HeadingLevel, AlignmentType as DocxAlign,
  BorderStyle, Header as DocxHeader, Footer as DocxFooter,
...
async function buildPRDDocx(initiative, foundation) {
  ...
  // Title banner — use background color via highlight workaround (shading not supported on Paragraph)
  paras.push(new DocxParagraph({
    children: [new DocxTextRun({ text: "PRODUCT REQUIREMENTS DOCUMENT", bold: true, size: 32, color: "1B3A6B" })],
    alignment: DocxAlign.CENTER,
    spacing: { before: 0, after: 160 },
    border: {
      bottom: { color: "1B3A6B", style: BorderStyle.SINGLE, size: 18, space: 1 },
    },
  }));
  ...
  paras.push(new DocxParagraph({ text: "", spacing: { before: 240, after: 0 } }));
  for (const [label, value] of metaItems) {
    paras.push(new DocxParagraph({
      children: [
        new DocxTextRun({ text: label + ":   ", bold: true, size: 22, color: "1B3A6B" }),
        new DocxTextRun({ text: value, size: 22 }),
      ],
  ...
  for (const line of lines) {
    ...
    if (!trimmed) {
      paras.push(new DocxParagraph({ text: "", spacing: { before: 60, after: 0 } }));
      continue;
    }
    if (trimmed.startsWith("### ")) {
      paras.push(new DocxParagraph({ text: trimmed.slice(4), heading: HeadingLevel.HEADING_3, spacing: { before: 240, after: 80 } }));
    } else if (trimmed.startsWith("## ")) {
      paras.push(new DocxParagraph({ text: trimmed.slice(3), heading: HeadingLevel.HEADING_2, spacing: { before: 360, after: 120 } }));
    } else if (trimmed.startsWith("# ")) {
      paras.push(new DocxParagraph({ text: trimmed.slice(2), heading: HeadingLevel.HEADING_1, spacing: { before: 480, after: 160 } }));
    } else if (trimmed.startsWith("- ") || trimmed.startsWith("* ")) {
      paras.push(new DocxParagraph({ text: trimmed.slice(2), bullet: { level: 0 }, spacing: { before: 40, after: 40 } }));
    } else {
      const parts = trimmed.split(/(\*\*[^*]+\*\*)/g);
      const runs = parts.map(p =>
        p.startsWith("**") && p.endsWith("**")
          ? new DocxTextRun({ text: p.slice(2, -2), bold: true, size: 22 })
          : new DocxTextRun({ text: p, size: 22 })
      );
      paras.push(new DocxParagraph({ children: runs, spacing: { before: 60, after: 60 } }));
    }
  }
  ...
  const doc = new DocxDocument({
    styles: {
      paragraphStyles: [
        { id: "Heading1", name: "Heading 1", run: { bold: true, size: 36, color: "1B3A6B" }, paragraph: { spacing: { before: 480, after: 160 } } },
        { id: "Heading2", name: "Heading 2", run: { bold: true, size: 28, color: "2E6DA4" }, paragraph: { spacing: { before: 360, after: 120 } } },
        { id: "Heading3", name: "Heading 3", run: { bold: true, size: 24, color: "1B3A6B" }, paragraph: { spacing: { before: 240, after: 80 } } },
      ],
    },
    sections: [{
      properties: {
      ...
      headers: {
        default: new DocxHeader({
          children: [new DocxParagraph({
            children: [
              new DocxTextRun({ text: (ini.slug || "") + " \u2014 PRD    ", size: 18, color: "6B7A99" }),
              new DocxTextRun({ text: "CONFIDENTIAL", size: 18, color: "D4A843", bold: true }),
            ],
      ...
      footers: {
        default: new DocxFooter({
          children: [new DocxParagraph({
            children: [
              new DocxTextRun({ text: (ini.title || "Initiative") + " \u00B7 Product Requirements Document", size: 16, color: "6B7A99" }),
            ],
